...
impl<T:EventType> TryFrom<T, &DynamoDBRecord> for Event {
  type Error = Error;
  ...
  fn try_from(value: &DynamoDBRecord) -> Result<Self, Self::Error> {
      match value.event_name.as_str() {
          "INSERT" => {
              let item<T> = (&value.dynamodb.new_image).try_into()?;
              Ok(Event::Created { item })
          }
          "MODIFY" => {
          ...
          "REMOVE" => {
              let item = (&value.dynamodb.old_image).try_into()?;
...
impl TryFrom<&HashMap<String, AttributeValue>> for Post {
  type Error = Error;
...
mod tests {
  ...
  fn test_dynamodb_into_event() {
      ...
      match &events[0] {
          Event::Created { post } => {
              ...
              assert_eq!(post.name, "new-item");
              assert_eq!(post.price, 10.5);
          }
          _ => {
              assert!(false)
          }
  ...
  #[test]
  fn test_dynamodb_into_post() {
      let ddb_event = get_ddb_event();

      let post: Post = (&ddb_event.records[0].dynamodb.new_image)
          .try_into()
          .unwrap();
